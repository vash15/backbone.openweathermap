;(function (window, factory) {
	...
	var OpenWeatherMap = Backbone.Model.extend({
		...
		// Alias name for Geo code
		fetchFromLatLng: function fetchFromLatLng(lat, lon, options){
			return this.fetchFromLatLon(lat, lon, options);
		},
		...
		getWeather: function getWeather(){
			...
			if ( weather.id ){
				...
				// Snow
				if ( Math.floor(weather.id/100) == 6 ){
					weather.category = "snow";
				}

				// rain
				if ( Math.floor(weather.id/100) == 3 || Math.floor(weather.id/100) == 5 ){
					weather.category = "rain";
				}

				// thunderstorm
				if ( Math.floor(weather.id/100) == 2 || [771, 781, 906, 902, 901, 900, 960, 961, 962].indexOf(weather.id) > -1  ){
			...
			weather.iconUrl = "";
			if ( weather.icon )
				weather.iconUrl = "//openweathermap.org/img/w/"+weather.icon+".png";
